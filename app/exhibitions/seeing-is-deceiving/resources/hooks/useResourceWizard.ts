...
export function useResourceWizard() {
  ...
  const getLibrarianLessonPlanPath = useCallback(() => {
    if (librarianGradeLevel && LIBRARIAN_LESSON_PLANS[librarianGradeLevel]) {
      return `/resources/seeing-is-deceiving/librarians/${librarianGradeLevel}/${encodeURIComponent(LIBRARIAN_LESSON_PLANS[librarianGradeLevel])}`;
    }
    return '';
  }, [librarianGradeLevel]);
  ...
  const getLibrarianHandoutPath = useCallback((level: 'green' | 'yellow' | 'red') => {
    if (librarianGradeLevel && LIBRARIAN_HANDOUTS[librarianGradeLevel]) {
      return `/resources/seeing-is-deceiving/librarians/${librarianGradeLevel}/${encodeURIComponent(LIBRARIAN_HANDOUTS[librarianGradeLevel][level])}`;
    }
    return '';
  }, [librarianGradeLevel]);
